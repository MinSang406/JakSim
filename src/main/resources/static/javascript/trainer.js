window.addEventListener('load', function(){
    ...
    if(window.location.href==='http://localhost:8080/timetable/res'){
        console.log('test');
        document.getElementById("manage_button").addEventListener("click", function() {
            var t_start_t = document.getElementById("t_start_t").value;
            var t_end_t = document.getElementById("t_end_t").value;
            var t_type = document.querySelector('input[type="radio"]:checked').value;
            var people_1 = document.getElementById("people_1").value;

            var reservation_display = t_start_t + "/" + t_end_t + "/" + t_type + "/" + people_1;
            console.log(reservation_display);
            document.getElementById("trainer_reservation_display").value = reservation_display;
        });
    }
